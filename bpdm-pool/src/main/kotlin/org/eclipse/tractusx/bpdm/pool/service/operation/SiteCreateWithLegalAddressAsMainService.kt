 ...
 * already backs another site ([org.eclipse.tractusx.bpdm.pool.model.LegalAddressAlreadyMainAddress]). `create` issues the site BPN and persists the site, then
 * reuses the legal address as its main address — so, unlike [SiteCreateService], it builds no new address, issues no
 * address BPN and emits no ADDRESS changelog. Order-preserving positional contract (see [org.eclipse.tractusx.bpdm.pool.model.ParseResult]).
 */
@Service
