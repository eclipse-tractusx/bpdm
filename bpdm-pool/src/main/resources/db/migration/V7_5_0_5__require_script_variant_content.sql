-- A script variant is a complete rendering of a business partner in another script. It therefore has to carry the
...
-- Renderings that carry no text at all in the field the contract requires.
...
-- Renderings of a name whose address counterpart does not exist. A legal entity or site script variant renders both
-- the name and the address of its script code, so one without the other cannot be represented any more.
