...
class LegalEntityIT @Autowired constructor(
    ...
    fun `create valid legal entity`(){
        //WHEN
        val legalEntityRequest = testDataV6Factory.request.createLegalEntityRequest(testName)
        val response = poolClient.legalEntities.createBusinessPartners(listOf(legalEntityRequest))

        //THEN
        val expectedLegalEntities = response.entities.map { testDataV6Factory.result.mapToExpectedLegalEntityCreate(legalEntityRequest) }
    ...
    fun `create valid legal entity and find it`(){
        ...
        //THEN
        val expectedSearchResponse = createSingleResultPage(testDataV6Factory.result.mapToExpectedLegalEntity(createdLegalEntity))
        assertRepo.assertLegalEntityGet(searchResponse, expectedSearchResponse)
    }
    ...
    fun `try create legal entity with duplicated identifier`(){
        //GIVEN
        val givenLegalEntityRequest = testDataV6Factory.request.createLegalEntityRequest("$testName 1")
        ...
        //WHEN
        val newLegalEntityRequest =  with(testDataV6Factory.request.createLegalEntityRequest("$testName 2")){
    ...
    fun `try create legal entities having duplicated identifiers`(){
        //WHEN
        val request1 = testDataV6Factory.request.createLegalEntityRequest("$testName 1")
        val sameIdentifier = request1.legalEntity.identifiers.first()
        val request2 = with(testDataV6Factory.request.createLegalEntityRequest("$testName 2")){
    ...
    fun `try create legal entity with unknown legalform`(){
        //WHEN
        val newLegalEntityRequest = with(testDataV6Factory.request.createLegalEntityRequest("$testName 1")){
    ...
    fun `try create legal entity with too many identifiers`(){
        //WHEN
        val newLegalEntityRequest = with(testDataV6Factory.request.createLegalEntityRequest(testName)){
