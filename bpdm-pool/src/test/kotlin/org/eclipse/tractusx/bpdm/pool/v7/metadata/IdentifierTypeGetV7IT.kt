...
class IdentifierTypeGetV7IT : UnscheduledPoolTestBaseV7() {

    /**
     * GIVEN identifier types
     * WHEN participant searches for legal entity identifier types
     * THEN all returned entries have the matching business partner type
     */
    @Test
    ...
    /**
     * GIVEN identifier types
     * WHEN participant searches for address identifier types
     * THEN all returned entries have the matching business partner type
     */
    @Test
     ...
     * WHEN participant searches for identifier types for that country
     * THEN participant gets the identifier type
     */
    @Test
    fun `find mandatory identifier types for country v7`() {
        //GIVEN
        val countryX = CountryCode.DE
        val identifierType = IdentifierTypeDto(
            testName, IdentifierBusinessPartnerType.LEGAL_ENTITY, testName, null, null, null, null,
            sortedSetOf(IdentifierTypeCategory.OTH),
     ...
     */
    @Test
    fun `find optional identifier types for country v7`() {
        //GIVEN
        val countryX = CountryCode.US
        val identifierType = IdentifierTypeDto(
            testName, IdentifierBusinessPartnerType.LEGAL_ENTITY, testName, testName, testName, testName, testName,
            sortedSetOf(IdentifierTypeCategory.OTH),
