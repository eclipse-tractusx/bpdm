...
class AddressGetV7IT : UnscheduledPoolTestBaseV7() {
    ...
    fun `get address by BPNA`() {
        ...
        val additionalAddressResponse = testDataClient.createAdditionalAddress(legalEntityResponse, testName)

        //WHEN
        val fetchedAddress = poolClient.addresses.getAddress(additionalAddressResponse.address.bpna)

        //THEN
        val expectedAddress = resultFactory.buildAddressSearchResponse(additionalAddressResponse)

        assertRepository.assertAddressGet(fetchedAddress, expectedAddress)
    }
