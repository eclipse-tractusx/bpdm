...
import org.eclipse.tractusx.bpdm.gate.client.service.businessPartner.GateClientBusinessPartner
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT, properties = ["bpdm.api.upsert-limit=2"])
@ActiveProfiles("test")
internal class LegalEntityControllerInputIT @Autowired constructor(
    ...
    val gateClientLegalEntity = GateClientLegalEntity(webClient)
    ...
    fun `upsert legal entities`() {
        ...
        try{
            gateClientLegalEntity.upsertLegalEntities(legalEntities)
    ...
    fun `get legal entity by external id`() {
        ...
        val legalEntity = gateClientLegalEntity.getLegalEntityByExternalId(SaasValues.legalEntity1.externalId.toString())
    ...
    fun `get legal entity by external id, not found`() {
        ...
        try{
            gateClientLegalEntity.getLegalEntityByExternalId("nonexistent-externalid123")
        } catch (e: WebClientResponseException){
    ...
    fun `get legal entity by external id, SaaS error`() {
        ...
        try{
            gateClientLegalEntity.getLegalEntityByExternalId(SaasValues.legalEntity1.externalId.toString())
        } catch (e: WebClientResponseException){
            val statusCode: HttpStatusCode = e.statusCode
            val statusCodeValue: Int = statusCode.value()
            assertTrue(statusCodeValue in 500..599)
...
//            .is5xxServerError
    ...
    fun `get legal entities`() {
        ...
        val pageResponse = gateClientLegalEntity.getLegalEntities(paginationValue)
...
//            .blockFirst()!!
    ...
    fun `get legal entities, SaaS error`() {
        ...
        try{
            ...
            gateClientLegalEntity.getLegalEntities(paginationValue)
        } catch (e: WebClientResponseException){
            val statusCode: HttpStatusCode = e.statusCode
            val statusCodeValue: Int = statusCode.value()
            assertTrue(statusCodeValue in 500..599)
...
//            .isBadRequest
    ...
    fun `validate a valid legal entity`() {
        ...
        wireMockServer.stubFor(
         ...
         val actualResponse = gateClientLegalEntity.validateLegalEntity(legalEntity)
