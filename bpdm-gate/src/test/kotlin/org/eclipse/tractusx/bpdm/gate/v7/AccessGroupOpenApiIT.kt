...
class AccessGroupOpenApiIT @Autowired constructor(
    ...
    @Test
    fun `an input manager is granted endpoints an input consumer is not`() {
        val inputConsumerOperations = operationsOf("v7-input-consumer")
        val inputManagerOperations = operationsOf("v7-input-manager")

        assertThat(inputManagerOperations.minus(inputConsumerOperations))
            .describedAs("endpoints granted to an input manager but not to an input consumer")
            .isNotEmpty()
    }

    @Test
    fun `input and output consumers are granted different endpoints`() {
        val inputConsumerOperations = operationsOf("v7-input-consumer")
        val outputConsumerOperations = operationsOf("v7-output-consumer")

        assertThat(inputConsumerOperations.minus(outputConsumerOperations))
            .describedAs("endpoints granted to an input consumer but not to an output consumer")
            .isNotEmpty()
        assertThat(outputConsumerOperations.minus(inputConsumerOperations))
            .describedAs("endpoints granted to an output consumer but not to an input consumer")
            .isNotEmpty()
    }
    ...
    companion object {
        private val ACCESS_GROUPS = listOf("v7-input-consumer", "v7-output-consumer", "v7-input-manager", "v7-admin")
    }
}
