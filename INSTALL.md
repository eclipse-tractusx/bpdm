...
#### ReadAccessPoolForUseCaseProvider
...
1. Create a policy of type `HasBusinessPartnerNumber` for the company's BPNL (if it not yet exists)
2. Create a policy of type `AcceptPurpose` with usage purpose for the pool (if not yet exists)
3. Create a technical user with role `BPDM Pool Use Case Provider` and the company's BPN identity
4. Create a `ReadAccessPoolForUseCaseProvider` asset with the created technical user for client credentials
5. Create a contract definition `ReadAccessPoolForUseCaseProvider` referencing the created asset
